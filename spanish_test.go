...
func TestSpanish_Months(t *testing.T) {
	settings := &Settings{Languages: []string{"es"}}
...
func TestSpanish_MixedWithEnglish(t *testing.T) {
	...
	settings := &Settings{Languages: []string{"es", "en"}}
...
func BenchmarkSpanish_SimpleDate(b *testing.B) {
	settings := &Settings{Languages: []string{"es"}}
	for i := 0; i < b.N; i++ {
		ParseDate("15 diciembre 2024", settings)
	}
}
...
func BenchmarkSpanish_RelativeDate(b *testing.B) {
	settings := &Settings{Languages: []string{"es"}}
	for i := 0; i < b.N; i++ {
		ParseDate("hace 2 días", settings)
	}
}
...
func BenchmarkSpanish_Weekday(b *testing.B) {
	settings := &Settings{Languages: []string{"es"}}
	for i := 0; i < b.N; i++ {
		ParseDate("próximo lunes", settings)
	}
}
