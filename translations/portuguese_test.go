...
func TestPortuguese_Months(t *testing.T) {
	settings := &godateparser.Settings{Languages: []string{"pt"}}
	...
	for _, tt := range tests {
		t.Run(tt.input, func(t *testing.T) {
			...
			if tt.wantMonth != 0 && result.Month() != tt.wantMonth {
				t.Errorf("godateparser.ParseDate(%q) month = %v, want %v",
					tt.input, result.Month(), tt.wantMonth)
			}
			if tt.wantDay != 0 && result.Day() != tt.wantDay {
				t.Errorf("godateparser.ParseDate(%q) day = %v, want %v",
...
func TestPortuguese_MixedWithEnglish(t *testing.T) {
	...
	settings := &godateparser.Settings{Languages: []string{"pt", "en"}}
...
func BenchmarkPortuguese_SimpleDate(b *testing.B) {
	settings := &godateparser.Settings{Languages: []string{"pt"}}
	for i := 0; i < b.N; i++ {
		_, _ = godateparser.ParseDate("15 dezembro 2024", settings)
	}
}
...
func BenchmarkPortuguese_RelativeDate(b *testing.B) {
	settings := &godateparser.Settings{Languages: []string{"pt"}}
	for i := 0; i < b.N; i++ {
		_, _ = godateparser.ParseDate("há 2 dias", settings)
	}
}
...
func BenchmarkPortuguese_Weekday(b *testing.B) {
	settings := &godateparser.Settings{Languages: []string{"pt"}}
	for i := 0; i < b.N; i++ {
		_, _ = godateparser.ParseDate("próxima segunda", settings)
	}
}
